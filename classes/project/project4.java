...
//basically binary serach tree me search krega..
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            System.out.println("13. Undo Last Student Delete");
            ...
            switch (choice) {
                ...
                case 4 -> {
                    ...
                    Student s = searchStudent(root, id);
                    if (s != null) {
                        deletedStudentsStack.push(new Student(s.id, s.name)); 
                        deletedStudentsStack.peek().roomNumber = s.roomNumber; 
                ...
                case 13 -> {
                    if (!deletedStudentsStack.isEmpty()) {
                        ...
                        root = insertStudent(root, s.id, s.name);
                        if (s.roomNumber != -1) {
                            if (!roomAllocation.containsKey(s.roomNumber)) {
                                roomAllocation.put(s.roomNumber, new ArrayList<>());
                            }
                            if (roomAllocation.get(s.roomNumber).size() < roomCapacity) {
                                roomAllocation.get(s.roomNumber).add(s);
                                searchStudent(root, s.id).roomNumber = s.roomNumber; // Link room
                            }
                        }
                        System.out.println("Undo successful. Student re-added: " + s.name);
                    } else {
                        System.out.println("No deleted student to undo.");
                    }
                }
